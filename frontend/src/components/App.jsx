...
function App() {
  ...
  useEffect(() => {
    if (localStorage.jwt) {
      Promise.all([api.getUserData(localStorage.jwt), api.getMovies(localStorage.jwt)])
  ...
  function editUserData(username, email){
    setIsSend(true)
    api.setUserInfo(username, email, localStorage.jwt)
      .then(res => {
        setCurrentUser(res)
        setIsSuccess(true)
        setIsEdit(false)
  ...
  function handleDeleteMovie(deletemovieId) {
    api.deleteMovie(deletemovieId, localStorage.jwt)
      .then(() => {
  ...
  function handleToggleMovie(data) {
    ...
    if (isAdd) {  
      ...
      api.addMovie(data, localStorage.jwt)
        .then(res => {
      ...
      <Routes>
        ...
        <Route path='/signup' element={
          loggedIn ? <Navigate to='/movies' replace /> :
            <Registration name={'signup'} onSignUp={handleRegister} setIsError={setIsError}/>
        } />
